from arithmetic import arithmetic_encode, arithmetic_decode, compare_files
...
while True:
    ...
    match k:
        ...
        case 1:
            ...
            encoded = input("Как будет называться файл? (без .ac): ") + ".ac"
            arithmetic_encode(original, encoded)
            print("Файл закодирован.")
        case 2:
            encoded = input("Введите название: ")
            decoded = input("Как будет называться файл? (без .txt): ") + "_decoded.txt"
            arithmetic_decode(encoded, decoded)
